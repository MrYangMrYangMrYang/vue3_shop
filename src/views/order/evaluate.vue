<!--
  @fileoverview 订单评价组件
  @module components/order/evaluate
  @description 负责用户对已完成订单的评价内容录入与发布提交，
               提供文本域输入与字数限制，评价成功后自动返回订单列表
  @requires stores/user
  @requires services/request
  @example
  // 路由配置: /order/evaluate?orderid=123 (需要登录)
  <router-link :to="{ path: '/order/evaluate', query: { orderid: 123 } }">去评价</router-link>
-->
<template>
  <div class="evaluate-page">
    <van-nav-bar title="发表评价" left-arrow @click-left="back" class="custom-nav" />
    ...
    <div class="evaluate-container">
      ...
      <div class="action-btn">
        <van-button round block type="primary" @click="sub">发布评价</van-button>
      </div>
    </div>
  </div>
</template>
