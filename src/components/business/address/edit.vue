...
<template>
  <div class="address-edit-page">
    <van-nav-bar title="编辑收货地址" left-arrow @click-left="back" class="custom-nav" />
...
const addrid = getRouteQueryValue(route.query, 'id', 0)
...
const del = async () => {
  ...
  showConfirmDialog({
    ...
    .then(async () => {
      deleting.value = true
      try {
        const result = await POST({ url: '/address/del', params: { id: addrid, busid: business.id } })
